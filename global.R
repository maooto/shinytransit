...
testcorrs <- c('NWR002', 'NWR014', 'NWR016')

restest <- spldf.mor@data[spldf.mor@data$corrcode %in% testcorrs, ]
...
for (l in 1:dim(restest)[1]) { 
  ...
  splist$latlist[[l]] <- fortlist[[testcorrs[l]]]$lat
  splist$longlist[[l]] <- fortlist[[testcorrs[l]]]$long
